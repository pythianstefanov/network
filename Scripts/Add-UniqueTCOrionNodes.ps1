...
$ExistingNodes = import-csv -path ".\existNodes.csv"

#$Nodes = import-csv -path ".\testAdd.csv"
#$ExistingNodes = import-csv -path ".\testExist.csv"

Foreach ($Node in $Nodes)

{

$UniqueNode = $ExistingNodes | where {$_.NodeIPAddress -ne $Node.NodeIPAddress}

  If ($UniqueNode)

  { 

	$NewNode = Orion-AddNode -SwisConnection $Swis -NodeName $Node.NodeName -NodeIPAddress $Node.NodeIPAddress -PollingEngineID $Node.PollingEngineID -PollingMethod $Node.PollingMethod -SNMPV2Community $Node.SNMPV2Community -City $Node.City
