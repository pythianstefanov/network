param ( 
    [parameter(Mandatory=$false)]$F5LTM,
    [parameter(Mandatory=$false)][switch]$allf5s,
    [parameter(Mandatory=$true)]$outputdir,
...
if($allf5s){
    $f5list = 	'10.80.64.38','10.30.64.38','10.80.0.38','10.30.0.38','10.10.12.5'

    $allvipsbiglist=@()
    foreach($F5LTM in $f5list){
    
Initialize-F5.iControl -Hostname $F5LTM -Credentials $cred
$ic = get-f5.iControl

  # Environment Variables
        $date = Get-Date -format M.d.yyyy
        $HTMLReport = "$outputdir\$date - $F5LTM - VIP Report.html"
        $CSVReport = "$outputdir\$date - $F5LTM - VIP Report.csv"
        $CSVcertReport = "$outputdir\$date - $F5LTM - certificate Report.csv"
        $ReportTitle = "VIP Report"
        

#we're just grabbing VIP Name, VIP IP, Service Port
...
for($i=0; $i -lt $AddressPortList.length; $i++) {
  ...
  Foreach($profile in ($ic.LocalLBVirtualServer.get_profile($VSList[$i]))){
  ...
  #create the object with the classifications into a nice usable object
     ...
     $result | Add-Member -type NoteProperty -Name 'F5LTM' -Value "$F5LTM"
     $result | Add-Member -type NoteProperty -Name 'VirtualServer' -Value "$virtual"
     $result | Add-Member -type NoteProperty -Name 'VirtualServer_enabled' -Value "$vipenabled"
     $result | Add-Member -type NoteProperty -Name 'VirtualServerStatus' -Value "$availability_status"
     $result | Add-Member -type NoteProperty -Name 'Status_Description' -Value "$availability_status_desc"
     $result | Add-Member -type NoteProperty -Name 'IP' -Value "$vaddr"
     $result | Add-Member -type NoteProperty -Name 'Port' -Value "$vport"
     $result | Add-Member -type NoteProperty -Name 'DefaultPool' -Value "$pool"
     $result | Add-Member -type NoteProperty -Name 'SSL Profile' -Value "$sslprofile"
...
$whoa = Join-Object -Left $allinfo -Right $allpoolinfo -LeftJoinProperty DefaultPool -RightJoinProperty Poolname -RightProperties 'PoolMembers','PoolMembersstatus'
...
$ResultSet = $whoa|  ConvertTo-Html -head $style -Title $ReportTitle -Body "<h1>$ReportTitle</h1>`n<h5>Updated: on $(Get-Date)</h5>"


    # Write Content to Report.
        Add-Content $HTMLReport $ResultSet
    
$whoa  |export-csv -NoTypeInformation -Path $CSVReport 
$allcerts |export-csv -NoTypeInformation -Path $CSVcertReport 

    $allvipsbiglist += $whoa
    }
    $allvipsbiglist |export-csv -NoTypeInformation -Path "$outputdir\$date - allvips Report.csv"
}
...
for($i=0; $i -lt $AddressPortList.length; $i++) {
  ...
  $virtual = $VSList[$i];
  write-output "checking VIP: $virtual"
  $vaddr = $AddressPortList[$i].address;
  $vport = $AddressPortList[$i].port;
  $pool = ($ic).LocalLBVirtualServer.get_default_pool_name( ($VSList[$i]) )
  #travelclick only uses the common partition, so lets remove it.
...
for($i=0; $i -lt $PoolList.length; $i++){
  $Poolmember = $ic.LocalLBPool.get_member_v2($PoolList[$i])
  $poolname =  ($PoolList[$i])
  write-output "checking pool: $poolname"
  if($poolmember){
        $Poolmembername = $poolMember.get_address()
        $PoolMemberstatuses = $ic.LocalLBPool.get_member_object_status($PoolList[$i], $Poolmember)
        $memberstatus = ($PoolMemberstatuses.availability_status)
