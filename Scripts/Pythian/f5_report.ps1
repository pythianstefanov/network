param ( 
    [parameter(Mandatory=$false)]$lb = ('10.80.64.38','10.30.64.38','10.30.0.38','10.80.0.38'),
...
#initialize all the things.
  ...
  # Environment Variables
        ...
        $HTMLReport = "$outputdir\$date - VIP Report.html"
        $CSVReport = "$outputdir\$date - VIP Report.csv"
        $CSVcertReport = "$outputdir\$date - certificate Report.csv"
        $ReportTitle = "VIP Report"

# $LB can be a list or one object, either way. we create one object with all data despite how many load balancers there are
$entirelist = @()
$certentirelist = @()
...
for($i=0; $i -lt $AddressPortList.length; $i++) {
  ...
  $virtual = $VSList[$i];
  write-output "checking VIP: $virtual on LB: $f5ltm"
  ...
  #create the object with the classifications into a nice usable object
     ...
     $result | Add-Member -type NoteProperty -Name 'LoadBalancer' -Value "$F5LTM"
...
for($i=0; $i -lt $PoolList.length; $i++){
  ...
  $poolname =  ($PoolList[$i])
  write-output "checking pool: $poolname on LB: $f5ltm"
  if($poolmember){
...
$ResultSet = $entirelist|  ConvertTo-Html -head $style -Title $ReportTitle -Body "<h1>$ReportTitle</h1>`n<h5>Updated: on $(Get-Date)</h5>"
    ...
    write-output "exporting csv's"

$entirelist  |export-csv -NoTypeInformation -Path $CSVReport 
$certentirelist |export-csv -NoTypeInformation -Path $CSVcertReport 