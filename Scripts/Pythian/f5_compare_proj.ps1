...
if($ldcdev){
    $LDCdevf5new = 	'10.80.40.11'
    $LDCdevf5old = '10.80.64.38'
    $f5list = $LDCdevf5new, $LDCdevf5old
    }


    $allvipsbiglist=@()
    for ($f=0; $f -lt $f5list.length; $f++){
    
Initialize-F5.iControl -Hostname ($f5list[$f]) -Credentials $cred
$ic = get-f5.iControl
...
for($i=0; $i -lt $AddressPortList.length; $i++) {
  ...
  #create the object with the classifications into a nice usable object
     ...
     $result | Add-Member -type NoteProperty -Name 'F5LTM' -Value ($f5list[$f]) 
...
$whoa = Join-Object -Left $allinfo -Right $allpoolinfo -LeftJoinProperty DefaultPool -RightJoinProperty Poolname -RightProperties 'PoolMembers','PoolMembersstatus'
    ...
    $new = $allvipsbiglist|where {$_.F5LTM -eq $f5list[0]}|select virtualserver
    $old = $allvipsbiglist|where {$_.F5LTM -eq $f5list[1]}|select virtualserver
