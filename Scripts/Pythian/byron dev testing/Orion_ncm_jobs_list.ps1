...
$2ndjoin |export-csv -Path "C:\pythian\Byron\output\$date - test out.csv"

### throwing in NCM job data requires pulling from this other table
$allncmjobs = Get-SwisData -SwisConnection $swis -Query 'SELECT NCMJobID, NCMJobName, NCMJobType, NCMJobCreator, NCMJobDefinitionXML, Enabled, DisplayName, Uri FROM Cirrus.NCM_NCMJobs'

$allncmjobs|export-csv -Path "C:\pythian\Byron\output\$date - test out.csv"